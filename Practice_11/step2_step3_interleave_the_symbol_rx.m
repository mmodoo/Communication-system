...
if RX_WITHOUT_SETTING == false
    ...
    sampling_freq = 20000; % Nyquist freq = sampling_freq / 2 임을 잊지 말아야
...
if RX_WITHOUT_SETTING == true
    ...
    % preamble 검출 잘 되는지 테스트 위한 코드 1줄
    rx_siganl_archieve = rx_signal;
    ...
    if WHETHER_BASIC_PILOT == true
        ...
        % Channel Estimation & Equalisation
        % rng('default');
        % pilot_freq = 2 * (randi([0, 1], N, 1) - 0.5);
        rng('default');
        temp_pilot = 2 * (randi([0, 1], N/2-1, 1) - 0.5);  % N/2-1 개만 생성
        pilot_freq = [0; temp_pilot; 0; flip(temp_pilot)];  % DC, 중간, 허미션 처리
        symbols_eq = {};
        for i = 1 : length(demode_OFDM_blks)
            if rem(i, 5) == 1
                % 수정: 채널 추정 시 수신한 블록을 pilot 신호로 나눕니다.
