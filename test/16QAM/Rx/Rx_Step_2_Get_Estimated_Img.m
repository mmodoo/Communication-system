function Estimated_Img = Rx_Step_2_Get_Estimated_Img(Rx_Setting_MAT_path_and_File_Name, Tx_signal, Base_WAV_Path_and_File_Name, Whether_median_filter, Whether_OCR, Whether_Use_Base_WAV_Changing_through_minute, Preamble)
    ...
    [~, idx] = max(xC);
    start_pt = lags(idx);
    % disp(['start_pt: ' num2str(start_pt)]);
    % disp(['Preamble length: ' num2str(length(Preamble))]);
    % disp(['Signal length after preamble removal: ' num2str(length(Tx_signal(start_pt + length(Preamble) + 1 : end)))]);

    samples_per_OFDM_block = N_cp__that_is__Cyclic_Prefix_Length + (N + 2);
    % disp(['Samples needed per OFDM block: ' num2str(samples_per_OFDM_block)]);
    % disp(['Total OFDM blocks needed: ' num2str(Total_OFDM_symbols_Number_that_is_including_Pilot)]);
    % disp(['Total samples needed: ' num2str(samples_per_OFDM_block * Total_OFDM_symbols_Number_that_is_including_Pilot)]);
    ...
    for i = 1 : Total_OFDM_symbols_Number_that_is_including_Pilot
        % disp(['Attempting to access up to index: ' num2str((N + 2) + N_cp__that_is__Cyclic_Prefix_Length)]);
    ...
    disp(['Length after pilot removal: ' num2str(length(symbols_est))]);
    
    % QPSK Demodulation
    % decoded_bits = (symbols_est + 1) / 2;
    % decoded_symbols = pskdemod(symbols_est, 8, pi/8);
    decoded_symbols = qamdemod(symbols_est, 16);
    disp(['Length after PSK demod: ' num2str(length(decoded_symbols))]);
    % decoded_bits = int2bit(decoded_symbols, 3);
    decoded_bits = int2bit(decoded_symbols, 4);
    disp(['Length after bit conversion: ' num2str(length(decoded_bits))]);
